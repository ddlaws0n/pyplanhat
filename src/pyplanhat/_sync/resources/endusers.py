...
class EndUsers(BaseResource):
    ...
    def list(self, company_id: str | None = None) -> list[EndUser]:
    ...
    def get(self, enduser_id: str) -> EndUser:
        ...
        """
        response = self._client.get(f"/endusers/{enduser_id}")
        data = self._handle_response(response)
        if data is None:
            raise InvalidRequestError("EndUser not found", 404, "")
        return EndUser(**data)

    def create(self, enduser: EndUser) -> EndUser:
    ...
    def update(self, enduser_id: str, enduser: EndUser) -> EndUser:
    ...
    def delete(self, enduser_id: str) -> None:
        """Delete an end user.
        ...
        Raises:
            InvalidRequestError: If the end user is not found.
        """
        response = self._client.delete(f"/endusers/{enduser_id}")
