...
from pyplanhat._sync.resources.conversations import Conversation
...
def test_list_conversations_success(async_client, httpx_mock):
    ...
    conversations = async_client.conversations.list()
...
def test_list_conversations_by_company(async_client, httpx_mock):
    ...
    conversations = async_client.conversations.list(company_id="comp-123")
...
def test_list_conversations_empty(async_client, httpx_mock):
...
def test_get_conversation_success(async_client, httpx_mock):
    ...
    conversation = async_client.conversations.get("conv-123")
...
def test_get_conversation_not_found(async_client, httpx_mock):
    ...
    with pytest.raises(InvalidRequestError) as exc_info:
        async_client.conversations.get("nonexistent")

    assert exc_info.value.status_code == 404
    assert "Conversation not found" in exc_info.value.response_body


def test_create_conversation_success(async_client, httpx_mock):
    ...
    created_conversation = async_client.conversations.create(new_conversation)
...
def test_create_conversation_validation_error(async_client, httpx_mock):
    ...
    with pytest.raises(APIError) as exc_info:
        async_client.conversations.create(conversation)

    assert exc_info.value.status_code == 400
    assert "Invalid request" in str(exc_info.value)


def test_update_conversation_success(async_client, httpx_mock):
    ...
    result = async_client.conversations.update("conv-123", updated_conversation)
...
def test_update_conversation_not_found(async_client, httpx_mock):
    ...
    with pytest.raises(InvalidRequestError) as exc_info:
        async_client.conversations.update("nonexistent", conversation)

    assert exc_info.value.status_code == 404
    assert "Conversation not found" in exc_info.value.response_body


def test_delete_conversation_success(async_client, httpx_mock):
    ...
    async_client.conversations.delete("conv-123")


def test_delete_conversation_not_found(async_client, httpx_mock):
    ...
    with pytest.raises(InvalidRequestError) as exc_info:
        async_client.conversations.delete("nonexistent")

    assert exc_info.value.status_code == 404
    assert "Conversation not found" in exc_info.value.response_body


def test_conversation_authentication_error(async_client, httpx_mock):
    ...
    with pytest.raises(AuthenticationError) as exc_info:
        async_client.conversations.list()

    assert exc_info.value.status_code == 401
    assert "Invalid API key" in exc_info.value.response_body


def test_conversation_rate_limit_error(async_client, httpx_mock):
...
def test_conversation_server_error(async_client, httpx_mock):
